...
echo "Making the CSR..."
openssl req -new -key ${CERT_NAME}.key -out ${CERT_NAME}.csr -config ./extras.conf

echo "Generating the certificate..."
openssl x509 -req \
                -in ${CERT_NAME}.csr \
                -CA ${CA_NAME}.cert -CAkey ${CA_NAME}.key -CAcreateserial \
                -out ${CERT_NAME}.cert \
                -days 9999 \
                -sha256
...
openssl x509 -text -in ${CERT_NAME}.cert
