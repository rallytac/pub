...
LINE_NUM=0
SESSION_TEMPLATE="{\"id\":\"@CURRENT_SESSION@\",\"enabled\":true,\"type\":0,\"groups\":[@SESSION_GROUPS@]}"
SESSION_TEMPLATE_WITH_DESCRIPTION="{\"id\":\"@CURRENT_SESSION@\",\"description\":\"@CURRENT_DESCRIPTION@\",\"enabled\":true,\"type\":0,\"groups\":[@SESSION_GROUPS@]}"
GROUP_TEMPLATE="{\
	\"id\":\"@GROUP_ID@\", \
	\"type\":1, \
...
function finalize_current_session_and_reset()
{
	...
	ALL_CONFIG="{\"meta\":{\"generator\":\"${META_HEADER}\"},\"sessions\":[${ALL_SESSIONS}],\"groups\":[${ALL_GROUPS}]}"
}
	...
	while read -r INPUT_LINE; do
		...
		if [ "${INPUT_LINE}" != "" ]; then
			...
			if [ "${FIRST_CHAR}" == "+" ]; then
				...
				SESSION_GROUPS="${SESSION_GROUPS}{\"id\":\"${GROUP_ID}\",\"allowAudio\": true, \"wantAudio\": true, \"allowText\": true, \"wantText\": true}"				
			elif [ "${FIRST_CHAR}" != "#" ]; then
