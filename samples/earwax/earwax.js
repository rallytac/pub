//
// Engage Activity Recorder REST Service (earwax)
...
server.listen(configuration.https.port, configuration.https.host, () => 
    {
        logI("---------------------------------------------------------------------------------------------------", true);
        logI("Engage Activity Recorder REST Service (earwax) " + VERSION, true);
        logI("Copyright (c) 2020 Rally Tactical Systems, Inc.", true);
        logI("", true);
...
function setupDatabase(fn)
{
    ...
    db.serialize(() => {
        ...
        db.run(`CREATE TABLE IF NOT EXISTS groups(
                    group_id CHAR(38) PRIMARY KEY NOT NULL,
                    group_name CHAR(38) NULL);`)

        db.run(`CREATE UNIQUE INDEX IF NOT EXISTS idx_groups_group_id
                    ON groups(group_id);`);

        db.run(`CREATE TABLE IF NOT EXISTS recordings(
                    item_key CHAR(32) PRIMARY KEY NOT NULL,
                    recording_node_id CHAR(38) NOT NULL,
                    event_id CHAR(38) NOT NULL,
                    group_id CHAR(38) NOT NULL,
                    type INT NOT NULL,
                    direction INT NOT NULL,
                    this_node_id CHAR(38) NULL,
                    ts_started DATETIME NOT NULL,
                    ts_ended DATETIME NULL,
                    node_id CHAR(38) NULL,
                    alias CHAR(16) NULL,
                    rxtx_flags INT NULL,
                    metadata TEXT NULL,
        ...
        db.run(`CREATE INDEX IF NOT EXISTS idx_recordings_recording_node_id
                        ON recordings(recording_node_id);`);

        db.run(`CREATE INDEX IF NOT EXISTS idx_recordings_group_id
                         ON recordings(group_id);`);

        db.run(`CREATE INDEX IF NOT EXISTS idx_recordings_type
                         ON recordings(type);`);
        db.run(`CREATE INDEX IF NOT EXISTS idx_recordings_direction
                         ON recordings(direction);`);

        db.run(`CREATE INDEX IF NOT EXISTS idx_recordings_ts_started
                         ON recordings(ts_started);`);

        db.run(`CREATE INDEX IF NOT EXISTS idx_recordings_ts_ended
                         ON recordings(ts_ended);`);

        db.run(`CREATE INDEX IF NOT EXISTS idx_recordings_node_id
                         ON recordings(node_id);`);

        db.run(`CREATE INDEX IF NOT EXISTS idx_recordings_alias
                         ON recordings(alias);`);
